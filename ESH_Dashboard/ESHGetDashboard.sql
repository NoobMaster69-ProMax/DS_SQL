...
CREATE OR ALTER PROCEDURE [dbo].[Dashboard_HumanResourceDashboard_ESH_Get_ChartData]
...
BEGIN
    ...
    -- Source table (ESH dashboard AGG)
    DECLARE @sourceTableName NVARCHAR(256) =
        N'[DSEFACTORY_AGG].dbo.[HumanResourceDashboard_ESH]';
    ...
    /* ============================================================
       ...
       - Build ORDER BY based on date/datetime columns (if any)
       - Otherwise default to [Year], MetricName (nice for this table)
       ============================================================ */
    INSERT INTO @StructureTable (name, type)
    SELECT
        name,
    ...
    ELSE
        SET @orderBy = N' ORDER BY [Year] ASC, [MetricName] ASC';

    /* ============================================================
       4) Build final WHERE clause from parsed conditions
       - Only allow safe operators
       - Only allow columns that exist in the source table
       ============================================================ */
